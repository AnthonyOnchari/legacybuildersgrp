...
async function loginWithPassword() {
    ...
    try {
        ...
        if (res.success && res.status === 'otp_sent') {
            ...
            // FIX: Store the name for later use
            if (res.name) {
                localStorage.setItem('legacy_temp_name', res.name);
            }
...
async function verifyOtpAndLogin() {
    ...
    try {
        ...
        if (res.success) {
            // FIX: Get the name from response or fallback
            let userName = res.name;
            if (!userName || userName.trim() === '') {
                // Try to get from stored temp name
                userName = localStorage.getItem('legacy_temp_name');
                if (!userName || userName.trim() === '') {
                    userName = pendingLoginEmail ? pendingLoginEmail.split('@')[0] : 'Member';
...
async function completeLogin(name, email) {
    // FIX: Ensure name is never undefined or empty
    if (!name || name.trim() === '') {
        name = email ? email.split('@')[0] : 'Member';
        name = name.charAt(0).toUpperCase() + name.slice(1);
        console.log('⚠️ Name was empty, using fallback:', name);
    }
